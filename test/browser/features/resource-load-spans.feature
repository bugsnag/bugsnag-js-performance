Feature: Resource Load Spans
  ...
  Scenario: Resource load spans are automatically instrumented (NPM build)
    ...
    And the trace payload field "resourceSpans.0.scopeSpans.0.spans.1" string attribute "http.url" matches the regex "^https:\/\/.*:[0-9]{4}\/favicon\.png\?height=100&width=100$"
  ...
  Scenario: Resource load spans are automatically instrumented (CDN build)
    ...
    And the trace payload field "resourceSpans.0.scopeSpans.0.spans.2" string attribute "http.url" matches the regex "^https:\/\/.*:[0-9]{4}\/favicon\.png\?height=100&width=100$"
